export function getCategory(): string {
  return `SELECT id, category_name AS name, category_description AS description, FROM categories WHERE id = $1`;
}

export function getCategories(): string {
  return `SELECT id, parent_id AS "parentId", category_name AS name, category_description AS description,
  json_build_object('image', image, 'placeholder', placeholder) as thumbnail FROM categories LIMIT $1 OFFSET $2`;
}
...
export function getCategoryForAdmin(): string {
  return `SELECT cate.id, cate.parent_id AS "parentId", cate.category_name AS name, cate.category_description AS description, cate.icon, 
  ...
  (SELECT json_build_object('id', parent_cate.id, 'name', parent_cate.category_name) FROM categories AS parent_cate WHERE parent_cate.id = cate.parent_id) AS parent,
...
export function getCategoriesForAdmin(): string {
  return `SELECT cate.id, cate.parent_id AS "parentId", cate.category_name AS name, cate.category_description AS description, cate.icon, cate.created_at AS "createdAt", cate.updated_at AS "updatedAt", 
  ...
  ARRAY((SELECT json_build_object('id', child_cate.id, 'parentId', child_cate.parent_id, 'name', child_cate.category_name, 
  'description', child_cate.category_description, 'icon', child_cate.icon, 'createdAt', child_cate.created_at, 'updatedAt', child_cate.updated_at,
  ...
  FROM categories AS cate WHERE cate.parent_id IS NULL ORDER BY $1 ASC LIMIT $2 OFFSET $3`;
}
...
export function getCategoriesParentsSelectForAdmin(): string {
  return `SELECT cate.id, cate.category_name AS name FROM categories AS cate WHERE cate.parent_id IS NULL ORDER BY $1 ASC LIMIT $2 OFFSET $3`;
}

export function getCategoriesParentsSelectForAdminWithId(): string {
  return `SELECT cate.id, cate.category_name AS name FROM categories AS cate WHERE cate.parent_id IS NULL and cate.id != $1 ORDER BY $2 ASC LIMIT $3 OFFSET $4`;
}

export function getCategoriesSelectAllForAdmin(): string {
  return `SELECT cate.id, cate.category_name AS name FROM categories AS cate ORDER BY $1 ASC LIMIT $2 OFFSET $3`;
}

// COALESCE((SELECT MAX(display_order)+1 FROM categories)
export function insertCategory(): string {
  return `
          INSERT INTO categories(parent_id, category_name, category_description, icon, image, placeholder, created_by) 
          VALUES($1, $2, NULLIF($3, ''), $4, NULLIF($5, ''), $6, $7) RETURNING id, category_name AS name
      `;
}

export function updateCategory(): string {
  return `
         UPDATE categories SET parent_id = $2, category_name = $3, category_description = $4, icon = $5, image = $6, placeholder = $7,  updated_by = $8
         WHERE id = $1 RETURNING id, category_name AS name
      `;
}
