...
export function getHeroCarouselListForAdmin(): string {
  return `SELECT slide.id, slide.title, slide.image as thumbnail, slide.clicks, slide.display_order AS "displayOrder", slide.published,  slide.created_at AS "createdAt", slide.updated_at AS "updatedAt", 
...
export function getHeroSlideForAdmin(): string {
  return `SELECT slide.id, slide.destination_url AS "destinationUrl", slide.image as thumbnail,
